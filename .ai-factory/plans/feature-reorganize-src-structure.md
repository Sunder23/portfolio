...
- [x] Task 13: Снять все временные `[refactor:*]`-логи, добавленные в задачах 1-12
- [x] Task 14: Прогнать полный набор тестов (`npm run test`) и билд/typecheck; исправить пропущенные импорты — 87/88 тестов зелёные, 1 фейл (`Footer.test.tsx`) идентичен main и не связан с рефакторингом (отсутствует Router/i18n wrapper в тесте); typecheck и production build зелёные
